<?php
if(!isset($_GET["data"]) || !isset($_GET["sec"])){
    ...
    $myfile = fopen($sec, "r") or die("Unable to open file!");
    $file_txt=fread($myfile,filesize($sec));
    fclose($myfile);
    ...
    foreach ($array as $char) {
        if($char!=' '){
            ...
            $mod=($nova_letra-$num)%26;
            if($mod<0){
                $nova_letra=$letter_array[26-abs($mod)];
            }else{
                $nova_letra=$letter_array[$mod];
            }
